...
class TargetConsistencyRunner02(BaseRunner):
    def __init__(self,
                 model,
                 cfg=None,
                 ...
                 max_iters=None,
                 max_epochs=None):
        super(TargetConsistencyRunner02, self).__init__(model,
                                                        batch_processor,
                                                        optimizer,
                                                        work_dir,
                                                        logger,
    ...
    def train(self, src_data_loader, tgt_data_loader):
        ...
        for i, src_data_batch in enumerate(self.data_loader):
            ...
            # ------------------------
            # forward network on source: task loss
            # ------------------------
            losses = self.model(**src_data_batch)  # forward; losses: {'seg_loss'=}

            # ------------------------
            # forward network on target
            # ------------------------
            if self.epoch >= self.cfg.target_start_epoch:
                # forward_target
                target_losses = self.model.forward_target(**tgt_data_batch)  # dict('seg_loss'=)
                losses['seg_target_loss'] = self.cfg.lambda_target * target_losses['seg_loss']
