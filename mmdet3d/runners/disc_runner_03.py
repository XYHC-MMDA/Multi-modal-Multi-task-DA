...
class DiscRunner03(BaseRunner):
    ...
    def train(self, src_data_loader, tgt_data_loader):
        ...
        for i, src_data_batch in enumerate(self.data_loader):
            ...
            seg_disc_prob = self.seg_disc(seg_fusion_feats)
            det_disc_prob = self.det_disc(det_fusion_feats)
            seg_disc_loss = self.seg_disc.loss(seg_disc_prob, src=True)
            det_disc_loss = self.det_disc.loss(det_disc_prob, src=True)
            ...
            if seg_acc > 0.6:
                losses['seg_src_loss'] = self.seg_disc.loss(seg_disc_prob, src=False)
            ...
            if det_acc > 0.6:
                losses['det_src_loss'] = self.det_disc.loss(det_disc_prob, src=False)
            ...
            seg_disc_prob = self.seg_disc(seg_fusion_feats)
            det_disc_prob = self.det_disc(det_fusion_feats)
            seg_disc_loss = self.seg_disc.loss(seg_disc_prob, src=False)
            det_disc_loss = self.det_disc.loss(det_disc_prob, src=False)
            disc_loss = seg_disc_loss + det_disc_loss
