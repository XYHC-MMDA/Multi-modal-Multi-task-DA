...
class MultiTaskCross(Base3DDetector):
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(MultiTaskCross, self).__init__()
    ...
    def extract_pts_feat(self, pts, img_feats, img, pts_indices, img_metas):
        """Extract features of points."""
        x = img.permute(0, 2, 3, 1)
        sample_rgb = []
        for i in range(x.shape[0]):
            sample_rgb.append(x[i][pts_indices[i][:, 0], pts_indices[i][:, 1]])
        concat_pts = []
        for i in range(len(pts)):
            concat_pts.append(torch.cat([pts[i], sample_rgb[i]], 1))

        voxels, num_points, coors = self.voxelize(concat_pts)  # voxels=(M, T=64, ndim=4+3); coors=(M, 4), (batch_idx, z, y, x)
    ...
    def extract_feat(self, points, pts_indices, img, img_metas):
        ...
        pts_feats = self.extract_pts_feat(pts=points, img_feats=img_feats, img=img, pts_indices=pts_indices, img_metas=img_metas)
    ...
    def init_weights(self, pretrained=None):
        ...
        super(MultiTaskCross, self).init_weights(pretrained)
